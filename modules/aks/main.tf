...
resource "azuread_group" "admin_group" {
  ...
  owners           = [data.azurerm_client_config.current.tenant_id]
...
module "aks" {
  ...
  node_pools = {
    "UserSpotPool" = {
      name                  = substr(replace(replace("spot-${local.normalized_cluster_name}", "-", ""), "_", ""), 0, 12)  
      ...
      vm_size               = var.aks_agent_config.system_pool_vm_size
